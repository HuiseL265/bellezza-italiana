		  ...
		  while ($row = $ctr->fetch_assoc())
		    {
              ...
              $valor1 = explode(":",$ct);
				if ($datediff>=0) {
  $n=((intval($valor1[0])-8)*4+(intval($valor1[1])/15));
					if ($datediff%$cr==0) {
						$events[$n]=$cn;
					}
...
for ($i=0; $i <sizeof($events) ; $i++) {
	...
	if ($events[$i]==null) {
echo "<tr><td >".$hr.":".$min."</td><td id=".$Year."/".$m."/".$day.'|'.$hr.':'.$min." class='event'><div style='width:226px;'></div>"."</td></tr>";
	}
	else{
echo "<tr><td >".$hr.":".$min."</td><td id=".$Year."/".$m."/".$day.'|'.$hr.':'.$min." class='eventadded'><div style='width:226px;' class='appt'>".$events[$i]."</div>"."</td></tr>";
}
